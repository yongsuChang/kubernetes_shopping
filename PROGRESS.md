...
- [ ] **웹 전용 레이아웃 최적화**: 현재 모바일 중심의 중앙 집중형 디자인을 넓은 화면(Web)에서 전체 폭을 활용하도록 개선
- [ ] **메인 페이지 고도화**:
    - 모든 권한 접근 가능
    - 최신 등록 상품 위주의 그리드 뷰
    - 필터 기능 (카테고리, 가격 등)
    - 페이지 이동을 위한 탭(Tab) 또는 사이드바(Sidebar) 내비게이션 강화

### 👤 고객 기능 (ROLE_USER)
- [ ] **주문 현황 상세 조회**: 주문 상태 변경 이력(History)을 타임라인 형태로 확인 가능
