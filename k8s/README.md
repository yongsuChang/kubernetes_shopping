...
> **※ 참고**: 위 다이어그램의 파드 배치는 이해를 돕기 위한 **논리적 예시**입니다. 실제 파드는 Kubernetes Scheduler에 의해 노드별 가용 리소스 상황에 맞춰 동적으로 배치됩니다. 특정 노드에 고정이 필요한 경우 `nodeSelector` 또는 `Affinity` 설정을 사용해야 합니다.
