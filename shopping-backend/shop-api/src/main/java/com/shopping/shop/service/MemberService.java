...
public class MemberService {
    ...
    public void signup(SignupRequest request) {
        ...
        if (memberRepository.existsByEmail(request.getEmail())) {
            throw new BusinessException("auth.email_exists");
        }
        ...
        if (request.getRole() == Role.ROLE_SHOP_ADMIN) {
            ...
            if (vendorRepository.existsByName(vName)) {
                ...
                throw new BusinessException("vendor.name_exists");
            }
    ...
    public LoginResponse login(LoginRequest request) {
        Member member = memberRepository.findByEmail(request.getEmail())
                .orElseThrow(() -> new BusinessException("auth.user_not_found"));

        if (!passwordEncoder.matches(request.getPassword(), member.getPassword())) {
            throw new BusinessException("auth.invalid_password");
        }
